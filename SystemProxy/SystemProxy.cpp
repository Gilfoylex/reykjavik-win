...
void reportWindowsError(LPCTSTR action)
{
    ...
    fwprintf(stderr, _T("Error %s: %lu %s\n"), action, errCode, pErrMsg);
}

void reportError(LPCTSTR action)
{
    fwprintf(stderr, _T("Error %s\n"), action);
}
...
int apply(INTERNET_PER_CONN_OPTION_LIST* options)
{
    ...
    if (dwRet == ERROR_BUFFER_TOO_SMALL)
    {
        ...
        {
            fwprintf(stderr, _T("Error RasEnumEntries: %d\n"), dwRet);
...
int SetSystemProxy(int method, const char* server)
{

    std::string cstr(server == nullptr ? "" : server);
    std::wstring wserver(cstr.begin(), cstr.end());
    ...
    if (method == 0) {
        ...
        options.pOptions[1].Value.pszValue = const_cast<LPWSTR>(wserver.data());

        options.pOptions[2].dwOption = INTERNET_PER_CONN_PROXY_BYPASS;
        options.pOptions[2].Value.pszValue = const_cast<LPWSTR>(_T("localhost;127.*;10.*;172.16.*;172.17.*;172.18.*;172.19.*;172.20.*;172.21.*;172.22.*;172.23.*;172.24.*;172.25.*;172.26.*;172.27.*;172.28.*;172.29.*;172.30.*;172.31.*;192.168.*"));
